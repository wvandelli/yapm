...
def decode_nodeinfo(info):
  ...
  ret.CPU = int(re.search('cpu MHz\s*:\s*(\d+)', cpuInfo).group(1))
  if len(cpufreq_safr) > 0: # then this should be a string with integer frequencies in kHz
    ...
    try:
      ...
      # this machine doesn't seem to support the scaling_available_frequencies string
  ...
  mem = int(float(re.search('MemTotal\s*:\s*(\d+)', memInfo).group(1)) / 1000.0)
  if ret.Memory > 65535:
  ...
  ret.Type = re.search('model name\s*:\s*(.+)', cpuInfo).group(1)
  ...
  regExp = re.compile('\s*(?P<LABEL>\S+).+HWaddr\s*(?P<MAC>\w\w:\w\w:\w\w:\w\w:\w\w:\w\w)\s+.+inet addr:(?P<IP>\d+\.\d+\.\d+\.\d+)')
  ret.Interfaces = []
